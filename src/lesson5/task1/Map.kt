...
fun whoAreInBoth(a: List<String>, b: List<String>): List<String> {
    ...
    for (x in a) if ((x in b) && (x !in c)) c.add(x)
    return c
}
...
fun canBuildFrom(chars: List<Char>, word: String): Boolean {
    ...
    for (i in chars.indices) chars[i].toLowerCase()
    if (w == chars.toSet()) return true
    return false
}
...
fun extractRepeats(list: List<String>): Map<String, Int> {
    ...
    for (i in list.indices) {
        if (list[i] !in map) map[list[i]] = 1
        else {
            map[list[i]] = map.getValue(list[i]) + 1
        }
    }
...
fun bagPacking(treasures: Map<String, Pair<Int, Int>>, capacity: Int): Set<String> {
    val res = mutableSetOf<String>()
    val masslist = mutableListOf<Int>()
    ...
    for ((k, v) in treasures) {
        pricelist.add(v.second)
        masslist.add(v.first)
        treasureslist.add(k)
    }
